...
func TestDocumentsTestExecution(t *testing.T) {
	...
	originalEvidenceLoader := loadTestDocsEvidence
	t.Cleanup(func() {
		newAIClientFromConfig = originalFactory
		loadTestDocsEvidence = originalEvidenceLoader
	})

	payload := data.Payload{Config: &sdkconfig.Config{}}
	loadTestDocsEvidence = func(payload data.Payload) (string, []string, error) {
	...
	t.Run("no AI config preserves legacy behavior", func(t *testing.T) {
		...
		if msg != testDocsFallbackMessage {
			t.Fatalf("message = %q, want %q", msg, testDocsFallbackMessage)
		}
	})
	...
	t.Run("client construction error falls back to needs review", func(t *testing.T) {
		...
		result, msg, _ := DocumentsTestExecution(payload)
		if result != gemara.NeedsReview || msg != testDocsFallbackMessage {
	...
	t.Run("ai returns pass verdict and records evidence", func(t *testing.T) {
		...
		result, msg, confidence := DocumentsTestExecution(collectingPayload)
	...
	t.Run("ai returns fail verdict", func(t *testing.T) {
		...
		result, _, confidence := DocumentsTestExecution(payload)
	...
	t.Run("ai provider error falls back and records no evidence", func(t *testing.T) {
		...
		result, msg, _ := DocumentsTestExecution(collectingPayload)
		if result != gemara.NeedsReview || msg != testDocsFallbackMessage {
			t.Fatalf("got (%v, %q), want legacy fallback", result, msg)
		}
...
func TestTestDocsEvidence(t *testing.T) {
	...
	if got := testDocsRootFile(payload, "readme"); got != "README.md" {
		t.Fatalf("testDocsRootFile(readme) = %q, want README.md", got)
	}
	...
	material, sources, err := testDocsEvidence(payload)
	if err != nil {
	...
	_, sources, err = testDocsEvidence(payload)
	if err != nil {
		t.Fatalf("unexpected evidence error: %v", err)
	}
	...
	if _, _, err := testDocsEvidence(data.Payload{}); err == nil {
